...
class TweetBottomSheet {
          ...
          var uri = await Utility.createLinkToShare(
            ...
            socialMetaTagParameters: SocialMetaTagParameters(
                description: model.description ??
                    "${model.user!.displayName} posted a tweet on TV Squared.",
                title: "Tweet on TV Squared app",
                imageUrl: Uri.parse(
  ...
  Widget _shareTweet(BuildContext context, FeedModel model, TweetType? type) {
    var socialMetaTagParameters = SocialMetaTagParameters(
        description: model.description ?? "",
        title: "${model.user!.displayName} posted a news on TV Squared.",
      ...
      children: <Widget>[
        ...
        _widgetBottomSheetRow(
          ...
          onPressed: () async {
            ...
            var uri = await url;
            Utility.share(uri.toString(), subject: "News");
          },
        ),
        const SizedBox(height: 8),
        _widgetBottomSheetRow(
          context,
          AppIcon.image,
          text: 'Share with News thumbnail',
          isEnable: true,
          onPressed: () {
            socialMetaTagParameters = SocialMetaTagParameters(
                description: model.description ?? "",
                title:
                    "${model.user!.displayName} posted a news on TV Squared.",
                imageUrl: Uri.parse(model.user?.profilePic ??
                    "https://play-lh.googleusercontent.com/e66XMuvW5hZ7HnFf8R_lcA3TFgkxm0SuyaMsBs3KENijNHZlogUAjxeu9COqsejV5w=s180-rw"));
            Navigator.pop(context);
            Navigator.push(
